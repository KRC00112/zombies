...
const goToMission = (setTransform, id, mapRef,mapContainerRef,MissionsList) => {
    const mission = MissionsList.find(l => l.id === id);
        ...
        <ul className='missions-list'>
            {MissionsList.map(obj => {
                return <li className={`mission-list-item ${selectedMission===obj.id?'mission-list-item-active':''}`} key={obj.id}>
                    <button className='mission-list-item-btn' onClick={() => {goToMission(setTransform, obj.id, mapRef,mapContainerRef,MissionsList);handleMissionSelect(obj.id)}}>{obj.name}</button><
                /li>
            })}
        </ul>
    </div>
...
function MapAndSelector({MissionsList,selectedMission,handleMissionSelect}) {
        ...
        >
            {(utils) => (
                    ...
                    <div className='map-and-missions'>
                        ...
                        <MissionsListBox {...utils}  mapRef={mapRef} mapContainerRef={mapContainerRef} handleMissionSelect={handleMissionSelect} selectedMission={selectedMission} MissionsList={MissionsList}/>
